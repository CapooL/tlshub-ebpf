...
本模块提供了全面的性能指标收集和分析功能。

建议测试的性能指标：
- **连接建立延迟**: 测量从连接开始到建立完成的时间
- **密钥协商时间**: 测量密钥协商过程的耗时
- **吞吐量**: 测量数据传输速率（Mbps）
- **CPU 使用率**: 监测程序的 CPU 使用情况
- **内存使用量**: 监测物理内存和虚拟内存使用情况

#### 自动性能监测

启动 capture 程序后，性能监测会自动启用：
...
1. **JSON 格式** (`perf_metrics_*.json`): 
   - 包含完整的统计摘要和每个连接的详细数据
   - 便于程序处理和自动化分析

2. **CSV 格式** (`perf_metrics_*.csv`):
   - 表格格式的连接数据
   - 便于在 Excel 等工具中查看和分析

#### 详细文档

完整的性能指标使用文档请参考：[../../docs/PERFORMANCE_METRICS_CN.md](../../docs/PERFORMANCE_METRICS_CN.md)
